...
describe('PatientService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw ConflictException if email already exists', async () => {
      ...
      repository.findByEmail.mockResolvedValue(mockPatient as any);

      await expect(service.create(createDto)).rejects.toThrow(ConflictException);
      expect(repository.findByEmail).toHaveBeenCalledWith(createDto.email);
    });
    it('should throw ConflictException if ssn already exists', async () => {
      const createDto = {
        email: 'test-email',
        firstName: 'test-firstName',
        lastName: 'test-lastName',
      ...
      repository.findBySsn.mockResolvedValue(mockPatient as any);

      await expect(service.create(createDto)).rejects.toThrow(ConflictException);
      expect(repository.findBySsn).toHaveBeenCalledWith(createDto.ssn);
    });
  });
